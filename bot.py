...
async def frontend_course(callback:CallbackQuery):   
    ...
    await callback.message.answer_photo(photo=image, caption="Bizning kurslarimizda siz Kop narsalarni organasiz", reply_markup=ortga)
    await callback.message.delete()
...
async def frontend_course(callback:CallbackQuery):   
    ...
    await callback.message.answer_photo(photo=image, caption="Navoiy viloyati\nNavoiy shahri\nPochtani tepasida", reply_markup=ortga_umumiy)
    await callback.message.delete()
    

@dp.callback_query(F.data == "back")
async def frontend_course(callback:CallbackQuery):   
    await callback.message.answer(text="Assalomu alaykum, foto surat yuboring", reply_markup=inline_menu)
    await callback.message.delete()
...
async def about_us(callback:CallbackQuery):
    await callback.message.answer(text="Qiluvchi: Abdalimov Asilbek", reply_markup=ortga_umumiy)
    await callback.message.delete()
    

@dp.callback_query(F.data == "contact-admin")
async def about_us(callback:CallbackQuery):
    await callback.message.answer_contact(first_name="Asilbek", phone_number="Abdalimov", reply_markup=ortga_umumiy)
    await callback.message.delete()  
